...
Renderer* renderer;
FlyCamera camera(glm::vec3(0.0f, 1.0f, 0.0f), glm::vec3(0.0f, 0.0f, -1.0f));
...
int main(int argc, char *argv[])
{
    ...
    */
    ...
    // LOG("Initializing render system");
        renderer = Init(window, (GLADloadproc)glfwGetProcAddress);
    ...
    // configure camera
    camera.SetPerspective(glm::Deg2Rad(60.0f), renderer->GetRenderSize().x / renderer->GetRenderSize().y ,0.1f, 100.0f);

    // scene setup
    SceneNode* mainTorus   = Scene::MakeSceneNode(&torus, matPbr);
    SceneNode* secondTorus = Scene::MakeSceneNode(&torus, matPbr);
    SceneNode* thirdTorus  = Scene::MakeSceneNode(&torus, matPbr);
    SceneNode* plasmaOrb   = Scene::MakeSceneNode(&tSphere, matPlasmaOrb);
    ...
    secondTorus->SetRotation(glm::vec4(0.0, 1.0, 0.0, glm::Deg2Rad(90.0)));
    ...
    PBRCapture *pbrEnv = renderer->GetSkypCature();
    ...
    // mesh 
    SceneNode* sponza = Resources::LoadMesh(renderer, "sponza", "meshes/sponza/sponza.obj");
    ...
    std::vector<glm::vec3> randomLightStartPositions;
    {
        for (int i = 0; i < 100; ++i)
        {
            ...
            light.Color = glm::vec3(Random::Uniliteral(), Random::Uniliteral(), Random::Uniliteral());
    ...
    while (!glfwWindowShouldClose(window))
    {
...

        {
            ...
            if (keysPressed[GLFW_KEY_W] || keysPressed[GLFW_KEY_UP])
                camera.InputKey(deltaTime, CAMERA_FORWARD);
            if (keysPressed[GLFW_KEY_S] || keysPressed[GLFW_KEY_DOWN])
                camera.InputKey(deltaTime, CAMERA_BACK);
            if (keysPressed[GLFW_KEY_A] || keysPressed[GLFW_KEY_LEFT])
                camera.InputKey(deltaTime, CAMERA_LEFT);
            if (keysPressed[GLFW_KEY_D] || keysPressed[GLFW_KEY_RIGHT])
                camera.InputKey(deltaTime, CAMERA_RIGHT);
            if (keysPressed[GLFW_KEY_E])
                camera.InputKey(deltaTime, CAMERA_UP);
            if (keysPressed[GLFW_KEY_Q])
                camera.InputKey(deltaTime, CAMERA_DOWN);
            ...
            for (int i = 0; i < randomLights.size(); ++i)
            {
                randomLights[i].Position = randomLightStartPositions[i] + glm::vec3(std::sin(glfwGetTime() * 0.31f + i * 3.17f) * 1.79f, std::cos(glfwGetTime() * 0.21f + i * 1.11f) * 1.61f, std::sin(glfwGetTime() * 0.49 + i * 0.79f) * 1.31);
            }
...
{
    renderer->SetRenderSize(width, height);
    camera.SetPerspective(glm::Deg2Rad(60.0f), (float)width / (float)height, 0.1f, 100.0f);
}
