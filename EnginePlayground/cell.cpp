...
Renderer* Init(GLFWwindow* window, GLADloadproc loadProcFunc)
{
    ...
    */
    ...
    {
        // LOG("Failed to initialize OpenGL context.", LOG_ERROR);
        return nullptr;
    }
    else
    {
        // LOG("OpenGL functions succesfully loaded.");
        // LOG("Version - Major: " + std::to_string(GLVersion.major) + " Minor: " + std::to_string(GLVersion.minor));
        // LOG("Driver: " + std::string((char*)glGetString(GL_VENDOR)) + " Renderer: " + std::string((char*)glGetString(GL_RENDERER)));
    }
    ...
    */
    ...
    {
        // LOG("Debug output not supported.");
    }
    ...
    void *userParam)
{
    ...
    if(msgCount++ < 3)
    {
        // LOG(logMessage, type == GL_DEBUG_TYPE_ERROR ? LOG_ERROR : LOG_WARNING);
    }
}
